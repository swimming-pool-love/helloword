...
def resultshow():
    return render_template('成果展示.html')

#partTwo.html的路由
@app.route('/partTwo/',methods=['GET'])
def partTwo():
    init()
    return render_template('partTwo.html', error=True)
...
def search():
    try:
        ...
        if keys not in ['']:
            ...
            if flag==0:
                return render_template('partTwo.html', error=False)
